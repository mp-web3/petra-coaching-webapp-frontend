...
export async function fetchLegalMarkdown(key: LegalDocKey, version: LegalVersion): Promise<string> {
    const base = key === 'terms' ? '/terms' : '/privacy'
    const url = `${base}/${version}.md`
    const res = await fetch(url, { cache: 'no-cache' })
    if (!res.ok) throw new Error(`Impossibile caricare il documento legale: ${key}@${version}`)
